...
from .gridbased_pooling import PoolingShape
...
class ShapeConfigLSTM(nn.Module):
    ...
    def forward(self, pooling_in, indexes):
        ...
        if self.state is None:
            raise Exception("The 'reset' method must be called prior to doing a forward pass")
        if self.shape == PoolingShape.GRID:
        ...
        if self.shape == PoolingShape.GRID:
            ...
            best_side = torch.tensor(self.sides, device=out.device).unsqueeze(0).repeat(idx.shape[0], 1).gather(
                1, idx.unsqueeze(1)).squeeze(1)
            self.all_sides[indexes] = best_side
            self.input[indexes, 0] = best_side
            self.curr_step_pp_probs_s = torch.cat(self.curr_step_pp_probs_s, prob)
        else:
            # shape == PoolingShape.ARC
